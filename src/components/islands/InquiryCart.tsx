    ...
    <>
      ...
      >
        {toasts.map((t) => (
          <div
            key={t.id}
            className="pointer-events-auto bg-primary text-primary-foreground text-sm rounded-none shadow-lg px-4 py-3 flex items-center gap-3 animate-in fade-in slide-in-from-bottom-2"
          >
  ...
  if (items.length === 0) {
    ...
    return (
      <div className="text-center py-16 border border-dashed border-border rounded-lg bg-secondary/10">
        ...
        <a href={href} className="inline-block px-6 py-3 bg-primary text-primary-foreground rounded-none font-medium hover:bg-primary/90">{L('emptyCta')}</a>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-4 border-t border-border">
        ...
        <div className="flex gap-3">
          ...
          <button type="button" onClick={sendInquiry} className="px-6 py-3 bg-primary text-primary-foreground rounded-none font-medium hover:bg-primary/90">{L('sendInquiry')} →</button>
        </div>
      </div>
    </div>
  );
