/**
 * POST /api/chat — AI 24h 智能客服 endpoint
 *
 ...
 *
 * Lives in src/pages/api/chat.ts so Astro 5 + @astrojs/vercel adapter
 * (output: 'server') recognizes it as an SSR endpoint and merges it into
 * the _render.func routes manifest. Vercel Node API's `api/` top-level
 * directory is NOT supported under this adapter.
 *
 * Handler written in Vercel-style (req, res) and wrapped through
 * toAstroApiRoute() shim so the existing business logic is unchanged.
 *
